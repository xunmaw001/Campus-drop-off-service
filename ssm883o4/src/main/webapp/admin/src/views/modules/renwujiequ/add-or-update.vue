<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="任务佣金" prop="renwuyongjin">
          <el-input v-model="ruleForm.renwuyongjin" 
              placeholder="任务佣金" clearable  :readonly="ro.renwuyongjin"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="任务佣金" prop="renwuyongjin">
              <el-input v-model="ruleForm.renwuyongjin" 
                placeholder="任务佣金" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="接取时间" prop="jiequshijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.jiequshijian" 
                type="datetime"
                :readonly="ro.jiequshijian"
                placeholder="接取时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.jiequshijian" label="接取时间" prop="jiequshijian">
              <el-input v-model="ruleForm.jiequshijian" 
                placeholder="接取时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="接取状态" prop="jiequzhuangtai">
          <el-select :disabled="ro.jiequzhuangtai" v-model="ruleForm.jiequzhuangtai" placeholder="请选择接取状态">
            <el-option
                v-for="(item,index) in jiequzhuangtaiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="接取状态" prop="jiequzhuangtai">
	      <el-input v-model="ruleForm.jiequzhuangtai"
                placeholder="接取状态" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.renwutupian" label="任务图片" prop="renwutupian">
          <file-upload
          tip="点击上传任务图片"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.renwutupian" label="任务图片" prop="renwutupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.renwutupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  created() {
	this.ruleForm.jiequshijian = this.getCurDateTime()
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `renwujiequ/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.renwujiequCrossAddOrUpdateFlag = false;
						 this.parent.search();
