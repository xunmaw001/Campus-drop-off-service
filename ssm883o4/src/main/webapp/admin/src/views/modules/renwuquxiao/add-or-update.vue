...
export default {
  ...
  methods: {
	...
	this.ruleForm.quxiaoyuanyin = this.ruleForm.quxiaoyuanyin.replace(reg,'../../../ssm883o4/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `renwuquxiao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.renwuquxiaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
