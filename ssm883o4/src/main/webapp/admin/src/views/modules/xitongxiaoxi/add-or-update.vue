<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="消息类型" prop="xiaoxileixing">
          <el-select :disabled="ro.xiaoxileixing" v-model="ruleForm.xiaoxileixing" placeholder="请选择消息类型">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="消息类型" prop="xiaoxileixing">
	      <el-input v-model="ruleForm.xiaoxileixing"
                placeholder="消息类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.xiaoxineirong = this.ruleForm.xiaoxineirong.replace(reg,'../../../ssm883o4/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xitongxiaoxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xitongxiaoxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
