...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussdaisongrenwuCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussdaisongrenwuCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussdaisongrenwu');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussdaisongrenwu');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussdaisongrenwu', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussdaisongrenwu', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussdaisongrenwu/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussdaisongrenwu', this.detail);
				this.$utils.msg('审核成功');
