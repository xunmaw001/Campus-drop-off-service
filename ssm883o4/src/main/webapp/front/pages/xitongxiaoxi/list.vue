<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.xiaoxileixing" type="text" placeholder="消息类型" ></input>
			</view>
        ...
        <view @tap="onDetailTap(product)" class="uni-product" :style='{"padding":"0px 20rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 1% 20rpx 2%","borderColor":"rgba(130, 163, 157, 1)","backgroundColor":"#fff","borderRadius":"20rpx","borderWidth":"2rpx","width":"47%","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
            ...
            <text v-if="userid && isAuth('xitongxiaoxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('xitongxiaoxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('xitongxiaoxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('xitongxiaoxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
      </view>
            <!-- 样式2 -->
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.xiaoxileixing){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`xitongxiaoxi`, params);
                } else {
                    res = await this.$api.list(`xitongxiaoxi`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xitongxiaoxi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.xiaoxileixing){
					searchForm['xiaoxileixing'] = '%' + this.searchForm.xiaoxileixing + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`xitongxiaoxi`, searchForm);
                } else {
                    res = await this.$api.list(`xitongxiaoxi`, searchForm);
                }
				// 如果是第一页数据置空
